		...
		if (count < int(data)):	
			...
			for s,p,o in graph2:
				if type(o)==rdflib.term.Literal and nif.isString in p:
					...
					pos=list(sentences.postags)
					for i,j in zip(sentence,pos):
						try:
							BI=o.index(i)
							EI=o.index(i)+len(i)
							...
							g.add([rdflib.term.URIRef("http://dbpedia.org/resource/"+name+"?dbpv=2016-10&nif=word_offset_"+str(BI)+"_"+str(EI)),nif.oliaLink,rdflib.term.URIRef("http://purl.org/olia/penn.owl#"+j)])
						except:
							pass
			g.bind("nif",nif)        
			g.serialize(destination='Files/POS/'+filename,format="turtle")
			count=count+1
print("Your Output is stored in POS Folder")
